...
def ongeki_rating_all(request):
    ...
    if request.method == "POST":
        ...
        # ベスト枠を算出
        filterd_records = [r for r in response["records"] if not (r["is_const_unknown"] or r["is_bonus"] or (r["const"] is None))]
        rating_all_best_songs_raw = sorted(filterd_records, key=lambda x: x[music_rate_type], reverse=True)[:30]
        music_rate_list = []
        ...
        # 情報収集
        ToolUsageManager.add_usage(request, "ongeki_rating_all", "")
